...
RSpec.describe "Users", type: :request do
  ...
  describe "POST /signup" do
    context "with matching password confirmation" do
      ...
      # it "saves the user id in the token" do
      #   post "/signup", params: user_params
      #   token = request.headers["Authorization"].split(" ")[1]
      #   expect(JWT.decode(token, ENV["SECRET"], true, { algorithm: "HS256" })[0]["user_id"]).to eq(User.last.id)
      # end
    ...
    context "with invalid data" do
      let!(:user_params) do 
        { 
          ...
          bio: "ndani ya kilemba kuna dredi noma!"
        } 
      end
  ...
  describe "POST/login" do
    let!(:user1) { User.create(username: "test_user_2", password: "chez3-chini", email:"test@example.com") }
    let!(:user2) { User.create(username: "test_user_1", password: "chez3-juu", email:"test-2@example.com") }
